...
use xilem_core::{Edit, InferHint, hint, lens, map_message};
...
fn elm_counter<T: 'static>(
    h: InferHint<Edit<T>, CountMessage>,
    count: i32,
) -> impl WidgetView<Edit<T>, CountMessage> {
    flex_col(
        h,
        (
            label(format!("elm count: {count}")),
            text_button(h, "+", |_| CountMessage::Increment),
            text_button(h, "-", |_| CountMessage::Decrement),
        ),
    )
}
...
fn map_message_counter(count: i32) -> impl WidgetView<Edit<i32>, CounterChanged> {
    hint!(h = Edit<i32>, CounterChanged);
    flex_row(
        h,
        (
            flex_col(
            ...
            flex_col(
                h,
                (
                    text_button(h, "reset all", |_| CounterChanged::Reset),
                    text_button(h, "do nothing (and don't rebuild the view tree)", |_| {
                        CounterChanged::Nop
                    }),
                ),
            ),
...
fn app_logic(state: &mut AppState) -> impl WidgetView<Edit<AppState>> + use<> {
    hint!(h = Edit<AppState>);
    flex_row(
        h,
        (
            map_action(
                elm_counter(hint!(Edit<AppState>, CountMessage), state.map_action_count),
